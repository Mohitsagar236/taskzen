    ...
    try {
      ...
      if (error) {
        ...
        if (data?.url) {
          ...
          try {
            // Use a safe way to construct the fallback URL
            const fallbackUrl = `${window.location.origin}/auth/v1/authorize?provider=google`;
            console.log('Attempting fallback authentication URL:', fallbackUrl);
