/*
  ...
  1. New Tables
    - `subscriptions`
      ...
      - Tracks plan type, status, and billing period
    - `subscription_features`
      - Defines features available for each plan
    - `subscription_usage`
      - Tracks feature usage for metered features

  2. Changes
    - Add subscription-related fields to users table
    - Add subscription checks to existing RLS policies
...
INSERT INTO subscription_features (plan, feature, limit) VALUES
  ('free', 'max_tasks', 100),
  ('free', 'max_projects', 1),
...
BEGIN
  ...
  -- If no limit (NULL) or no feature found, allow
  ...
  RETURN (current_usage + increment) <= feature_limit;
END;
$$;
